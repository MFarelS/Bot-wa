...
async function ephoto(url, text){
	...
	const token2 = /<div.*?id="form_value_input".+>(.*?)<\/div>/.exec(caritoken2)
	// if (!token2) throw new Error('Token Tidak Ditemukan!!')
	const prosesimage = await post('https://en.ephoto360.com/effect/create-image', JSON.parse(token2[1]), hasilcookie)
